...
namespace Shape_AI
{
    internal class DataPrep
    {
        ...
        //Takes the first / only file in the UserInput directory and stores this in a bitmap
        {
            ...
            string UserInputFilepath = Environment.GetFolderPath(Environment.SpecialFolder.Desktop) + @"\DEMO\Shape_AI\Data\UserInput\";
        ...
        //Stores the contoured bitmap into a 2 dimensional array
        {
            ...
            for (int i = 0; i < UserImage.Width; i++)           //i = x coordinate
            {
                for (int j = 0; j < UserImage.Height; j++)      //j = y coordinate
                {
                    ...
                    //Debug.WriteLine(UserDataArray[i, j]);
                }
            }

            //Debug.WriteLine(UserDataArray.Length);
        ...
        //Writes the 2D array to a textfile for retrieval later on
        {
            ...
            string UserDataArrayEncodedFilepath = Environment.GetFolderPath(Environment.SpecialFolder.Desktop) + @"\DEMO\Shape_AI\Data\EncodedData\EncodedData.txt";
...
/*
https://docs.microsoft.com/en-us/dotnet/api/system.drawing.bitmap.getpixel?view=dotnet-plat-ext-6.0
https://www.youtube.com/watch?v=bqEBIKbtm7Q
